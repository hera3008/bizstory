...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 링크, 버튼
	$link_list = $local_dir . "/bizstory/test/login_list.php"; // 목록
	$link_form = $local_dir . "/bizstory/test/login_form.php"; // 등록
	$link_ok   = $local_dir . "/bizstory/test/login_ok.php";   // 저장
?>
<div class="info_text">
	<ul>
		<li>년, 내용은 꼭 입력해주세요.</li>
	</ul>
</div>
...
//------------------------------------ 등록
	function check_form()
	{
		...
		chk_value = $('#post_hi_year').val();
		chk_title = $('#post_hi_year').attr('title');
		if (chk_value == '')
		{
		...
		if (action_num == 0)
		{
			...
			$.ajax({
				...
				success: function(msg) {
					...
					else if (msg.success_chk == "no_login")
					{
						$("#loading").fadeOut('slow');
						$("#backgroundPopup").css({"opacity": "0.7",'width':'100%','height':'100%'}).fadeIn("slow");
						check_auth_popup(msg.error_string);
						popupform_login(msg.fmode, msg.smode);
					}
					else
					{
						$("#loading").fadeOut('slow');
