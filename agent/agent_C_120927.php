		...
		foreach ($button_list as $k => $data)
		{
			if (is_array($data))
			{
				...
				else
				{
					if ($btn_type != '')
					{
						$button_url[$sort]    = 'javascript:void(0);" onclick="' . $set_agent_button_url[$btn_type];
						$button_target[$sort] = '';
					}
					else
					{
...
?>
		<div id="agent_icon">
			...
			<div>
				<ul>
					<li class="icon01"><a href="javascript:void(0);" onclick="move_receipt('');"><?=$button_name[1];?></a></li>
					<li class="icon02"><a href="<?=$button_url[2];?>"<?=$button_target[2];?>><?=$button_name[2];?></a></li>
				</ul>
				<ul>
					<li class="icon03"><a href="<?=$button_url[3];?>"<?=$button_target[3];?>><?=$button_name[3];?></a></li>
					<li class="icon04"><a href="<?=$button_url[4];?>"<?=$button_target[4];?>><?=$button_name[4];?></a></li>
				</ul>
			</div>
		</div>
	...
	?>
		...
		?>
							<li class="news-item"><span class="maintain_notice">[Bizstory] <?=$content;?><?=$important_span;?></span></li>
		<?
					}
				}
...
?>
	...
	// SlideShow
		...
		function move_secret()
		{
			$('#agent_sub_type').val('');
			document.agentform.action = 'secret.php';
			document.agentform.method = 'post';
			document.agentform.target = 'receipt_popup';
			//document.agentform.submit();
		}
