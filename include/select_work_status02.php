...
<div class="pstatus_box">
	<div class="pstatus_top">
		<p class="count">업무를 진행하시겠습니까?</p>
	</div>

	<form id="status02form" name="status02form" action="<?=$this_page;?>" method="post">
		<input type="hidden" id="status02_comp_idx" name="comp_idx" value="<?=$code_comp;?>" />
		<input type="hidden" id="status02_part_idx" name="part_idx" value="<?=$code_part;?>" />
		<input type="hidden" id="status02_wi_idx"   name="wi_idx"   value="<?=$wi_idx;?>" />
		<input type="hidden" id="status02_sub_type" name="sub_type" value="status02" />
		...
		?>
			...
			<div class="pstatus_wrap">
				<div class="pstatus_data">
					<div class="user_edit">
						대기인 업무를 진행시키려면 담당자와 기한을 설정해야 합니다.
					</div>
				</div>
			</div>
	...
	function form_workstatus()
	{
		...
		if (action_num == 0)
		{
			$.ajax({
				...
				data: $('#status02form').serialize(),
				success: function(msg) {
					if (msg.success_chk == "Y")
					{
						popup_work_close();
						list_data();
						view_open($('#status02_wi_idx').val());
					}
