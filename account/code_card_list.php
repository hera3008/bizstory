	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				$sort_data = query_view("select min(sort) as min_sort, max(sort) as max_sort from code_account_card where del_yn = 'N' and comp_idx ='" . $code_comp . "' and part_idx ='" . $code_part . "'");
...
?>
			...
			<td><?=$set_card_gubun[$data["card_gubun"]];?></td>
			<td><div class="left"><?=$code_name;?>(<?=$data['card_num'];?>) - <?=$data['mem_name'];?></div></td>
