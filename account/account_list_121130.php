	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				{
					$add_content = '<span class="eng">추가내용-' . $data['card_code_name'] . '(' . $data['card_mem_name'] . ': ' . $data['card_num'] . ')</span>';
				}
				else if ($data['gubun_code'] == 'bank') // 계좌이체일 경우
				{
					$add_content = '<span class="eng">추가내용-' . $data['bank_code_name'] . '(' . $data['bank_num'] . ')</span>';
				}
?>
		<tr>
