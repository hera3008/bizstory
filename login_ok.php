<?
	...
	include $local_path . "/bizstory/common/no_direct.php";
...
// 로그인
	function check_login()
	{
		...
		$mem_where = " and cu.mem_id = '" . $param["mem_id"] . "'";
		...
		else
		{
			...
			$mem_where = " and cu.mem_id = '" . $param["mem_id"] . "' and cu.mem_pwd = '" . $mem_pwd . "'";
			$mem_data = client_user_data("view", $mem_where);
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							{
								client_login_action($mem_data["cu_idx"], $sess_str);
							...
							// 최종방문일, 카운트 하기
								db_query("
									...
									where
										cu_idx = '" . $mem_data["cu_idx"] . "'
								");

								$move_url = urldecode($move_url);
								if ($move_url == '') $move_url = '/cms/receipt.php';
								$move_url = '/cms/receipt.php';

								$str = '{"success_chk" : "Y","move_url" : "' . $move_url . '"}';
								echo $str;
								exit;
							}
						}
