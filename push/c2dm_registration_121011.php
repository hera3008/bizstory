...
$log_file = "log/android_registration.".date("Ymd").".log";
$log = new FileLog("c2dm_registration");
...
if (count($_list) > 0)
{
	...
	if ($cmd == "add") {
		...
		if (count($_list) > 0)
		{
			$sql = "update push_member set push_enable = 'Y', push_message = '$message', push_receipt = '$receipt', push_work = '$work', push_notice = '$notice', push_registration_id = '$registration_id', push_device_unique_id = '$device_unique_id', mod_date = now() where push_id = '$id' and push_device_type = 'A' ";
		}
		else
		{
			$sql = "insert into push_member (comp_idx, part_idx, mem_idx, push_id, push_pwd, push_name, push_enable, push_device_type, push_registration_id, "
				 ...
				 . "values ('$comp_idx','$part_idx','$mem_idx','$id','$pwd','$mem_name','Y','A','$registration_id','$message','$receipt','$work','$notice','$device_unique_id',now(),'N')";
		}
		$log->log("sql = ".$sql);
		$db->__execute($sql);
	}
	...
	if ($cmd == "del") {
		...
		if (count($_list) > 0)
		{
			$sql = "update push_member set push_enable = 'N', push_message = '$message', push_receipt = '$receipt', push_work = '$work', push_notice = '$notice', push_registration_id = '$registration_id', push_device_unique_id = '$device_unique_id', mod_date = now() where push_id = '$id' and push_device_type = 'A' ";
		}
		$log->log("sql = ".$sql);
		$db->__execute($sql);
	}
	...
	if ($cmd == "registration") {
		$sql = "select * from push_member where push_id = '$id' and push_pwd = '$pwd' and push_device_type = 'A' ";
		$log->log("sql = ".$sql);
		$_list = $db->__list($sql);
		if (count($_list) > 0)
		{
			$sql = "update push_member set push_registration_id = '$registration_id', mod_date = now() where push_id = '$id' and push_device_type = 'A' ";
		}
		$log->log("sql = ".$sql);
		$db->__execute($sql);
	}
