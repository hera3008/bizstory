...
if (count($_list) > 0)
{
	...
	if ($cmd == "add") {
		...
		if (count($_list) > 0)
		{
			$sql = "update push_member set push_enable = 'Y', push_message = '$message', push_receipt = '$receipt', push_work = '$work', push_notice = '$notice', push_device_unique_id = '$device_unique_id', mod_date = now() where push_id = '".$id."'";
		}
		else
		{
			$sql = "insert into push_member (comp_idx, part_idx, mem_idx, push_id, push_pwd, push_name, push_enable, push_registration_id, "
				 ...
				 . "values ('$comp_idx','$part_idx','$mem_idx','$id','$pwd','$mem_name','Y','$registration_id','$message','$receipt','$work','$notice','$device_unique_id',now(),'N')";
		}
		$log->log("sql = ".$sql);
		$db->__execute($sql);
	}
	...
	if ($cmd == "del") {
		...
		if (count($_list) > 0)
		{
			$sql = "update push_member set push_enable = 'N', push_message = '$message', push_receipt = '$receipt', push_work = '$work', push_notice = '$notice', push_device_unique_id = '$device_unique_id', mod_date = now() where push_id = '".$id."'";
		}
		$log->log("sql = ".$sql);
		$db->__execute($sql);
	}
