...
class C2DM {
	...
	// 수신자 정보를 확인한다.
		$sql = "select * from push_member where del_yn = 'N' and push_id = '" . $receiver . "' and push_device_type = 'A'";
		//$log->log("sql = ".$sql);
		$_list = $db->__list($sql);
		if (count($_list) > 0)
		{
			...
			//$log->log("[".$row['push_id']."] message = ".$row['push_message'].", receipt = ".$row['push_receipt'].", work = ".$row['push_work'].", notice = ".$row['push_notice'].", consult = ".$row['push_consult']);

		// 메시지 타입에 대한 전송설정 상태를 확인한다.
			switch ($msg_type)
			{
				case "message":
					if ($row['push_message'] == 'Y') $send = 1;
					else
					{
				...
				case "consult": // 상담게시판
					if ($row['push_consult'] == 'Y') $send = 1;
					else
					{
						//$log->log($receiver." [".$receiver_name."] 상담알림 사용하지 않음");
						$state = "0";
						$state_comment = "상담알림 사용하지 않음";
					}
					break;
