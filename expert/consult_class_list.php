...
// 변수
	...
	$where = "";
	$list = expert_code_consult_class_data('list', $where, '', '', '');
?>
...
<?
	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				$sort_data = query_view("select min(sort) as min_sort, max(sort) as max_sort from expert_code_consult_class where del_yn = 'N'");
