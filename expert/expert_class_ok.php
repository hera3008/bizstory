...
// 등록 함수
	...
	// 기본값설정
		if ($param['default_yn'] == 'Y')
		{
			...
			$page_data = expert_code_expert_class_data("page", $where);
	...
	// 등록
		$command    = "insert"; //명령어
		$table      = "expert_code_expert_class"; //테이블명
		$conditions = ""; //조건
...
//위 정렬 함수
	function sort_up()
	{
		...
		$data = expert_code_expert_class_data('view', $where);

		$sort_where = " and code.sort < '" . $data["sort"] . "'";
		$sort_order = "code.sort desc";
		$prev_data = expert_code_expert_class_data('view', $sort_where, $sort_order);
...
//아래 정렬 함수
	function sort_down()
	{
		...
		$next_data = expert_code_expert_class_data('view', $sort_where, $sort_order);
