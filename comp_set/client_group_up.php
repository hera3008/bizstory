...
*/
	...
	else
	{
		...
		{
?>
	<select name="param[menu<?=$i;?>]" id="chk_menu<?=$i;?>" title="<?=$i?>단계 분류단계를 선택해 주세요"  class="form-select form-select-sm" 
		data-control="select2" data-hide-search="true" data-placeholder="<?=$i?>단계 분류단계를 선택해 주세요" aria-label="<?=$i?>단계 분류단계를 선택해 주세요" title="<?=$i?>단계 분류단계를 선택해 주세요"
...
?>
		<option value="<?=$menu_data["ccg_idx"];?>" data-receipt-code="<?=$menu_data["receipt_code"];?>" <?=selected($chk_idx, $menu_data["ccg_idx"]);?>><?=$menu_data["group_name"];?></option>
<?
				}
			}
?>
