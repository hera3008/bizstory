...
//기초값 검사
	...
	{
	//필수검사
		$chk_param['require'][] = array("field"=>"client_name", "msg"=>"거래처");
...
// 등록 함수
	function post()
	{
		...
		$data = query_view("select max(ci_idx) as ci_idx from " . $table);
		$param['ci_idx'] = ($data['ci_idx'] == "") ? '1' : $data['ci_idx'] + 1;

	// 최고관리자 아이디구하기
		$mem_where = "and mem.comp_idx = '" . $comp_idx . "' and mem.ubstory_level = '11'";
		$mem_data = member_info_data('view', $mem_where);
		$group_mem_id = $mem_data['mem_id'];

	// 거래처코드구하기
		$client_code_data = query_view("select max(client_code) as client_code from " . $table . " where comp_idx='" . $comp_idx ."'");
		$code_date = ($client_code_data["client_code"] == "") ? "1" : $client_code_data["client_code"] + 1;
		
		$client_code = strlen($comp_idx.$code_date) < 10 ? $comp_idx.sprintf('%0'.(10-strlen($comp_idx)).'d',$code_date) : $comp_idx.$code_date;
	...
	// 아이피
		...
		for ($i = 1; $i <= count($_POST['receipt_class']); $i++)
		{	
			$alarm_param['ci_idx']			= $param['ci_idx'];
			$alarm_param['comp_idx']		= $param['comp_idx'];
			$alarm_param['comp_client_idx']	= $param['comp_client_idx'];			
			$alarm_param['receipt_class']	= $_POST['receipt_class'][$i];

			$client_code_data = query_view("select receipt_code from code_receipt_class where code_idx='" . $alarm_param['receipt_class'] ."'");
			$alarm_param['receipt_code']	= $client_code_data['receipt_code'];

			$alarm_param['client_code']		= $param["client_code"];
			$alarm_param['charge_mem_idx']	= $_POST['charge_mem_idx'][$i];
			$alarm_param['charge_mem_name']	= $_POST['charge_mem_name'][$i];
			$alarm_param['charge_tel_num']	= $_POST['charge_tel_num'][$i];
			$alarm_param['charge_hp_num']	= $_POST['charge_hp_num'][$i];
			$alarm_param['charge_email']	= $_POST['charge_email'][$i];
			$alarm_param['reg_id']			= $param['reg_id'];
			$alarm_param['reg_date']		= date("Y-m-d H:i:s");			

			$receipt_alarm_data = query_view("select count(cra_idx) as cnt from clent_receipt_alarm where comp_idx='". $alarm_param['comp_idx'] ."' and receipt_class='" . $alarm_param['receipt_class'] ."'");
			if($receipt_alarm_data['cnt'] > 0)
			{
				$cra_idx	= $_POST['cra_idx'][$i];
				$command    = "update"; //명령어
				$table      = "clent_receipt_alarm"; //테이블명
				$conditions = "cra_idx = '" . $alarm_param['cra_idx'] . "'"; //조건
			}
			else
			{
				$alarm_command    = "insert"; //명령어
				$alarm_table      = "clent_receipt_alarm"; //테이블명
				$alarm_conditions = ""; //조건;
			}

			$alarm_query_str = make_sql($alarm_param, $alarm_command, $alarm_table, $alarm_conditions);			
			db_query($alarm_query_str);
			query_history($alarm_query_str, $alarm_table, $alarm_command);
			
		}


	// 거래처코드 업데이트
		//$query_str = "update client_code_info set ci_idx = '" . $param['ci_idx'] . "', part_idx = '" . $param['part_idx'] . "' where cci_idx = '" . $client_code_data['cci_idx'] . "'";
		//db_query($query_str);
		//query_history($query_str, 'client_code_info', 'update');
