...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 링크, 버튼
	$link_list         = $local_dir . "/bizstory/comp_set/client_list.php";      // 목록
	$link_form         = $local_dir . "/bizstory/comp_set/client_form.php";      // 등록폼
	$link_ok           = $local_dir . "/bizstory/comp_set/client_ok.php";        // 저장
	$link_excel        = $local_dir . "/bizstory/comp_set/client_excel.php";     // 액셀
	$link_print        = $local_dir . "/bizstory/comp_set/client_print.php";     // 인쇄
	$link_print_detail = $local_dir . "/bizstory/comp_set/client_print_sel.php"; // 상세인쇄
	...
	$where = " and comp.comp_code = '" . $comp_code . "'";
	$comp_data = company_info_data('view', $where);
	if($comp_data['total_num'] == 0) 
		$comp_client_idx = "";
	else 
		$comp_client_idx = $comp_data['comp_idx'];
                                ...
                                <div class="card card-flush">
                                    <div
                                        ...
                                        <div class="card-title">
                                            <h4 class="fs-1">거래처 <?=!$sub_type? '목록' : ($sub_type == 'postform' || $sub_type == 'modifyform' ? '등록' : '내용')?></h4>
                                        </div>
...
//------------------------------------ 등록, 수정
	function check_form()
	{
		...
		chk_value = $('#post_ccg_idx').val(); // 거래처명
		chk_title = $('#post_ccg_idx').attr('title');
		if (chk_value == '')
		{
			chk_total = chk_total + chk_title + '<br />';
			action_num++;
