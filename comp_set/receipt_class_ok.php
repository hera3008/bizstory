...
// 등록 함수
	...
	// 기본값설정
		...
		{
			$where = " and code.default_yn='Y'" . (!$comp_idx && !$part_idx ? " and code.import_yn = 'Y'":"");
			$where = $where .($comp_idx ? " and code.comp_idx = '" . $comp_idx . "'" : "");
			$where = $where .($part_idx ? " and code.part_idx = '" . $part_idx . "'" : "");
	...
	// 처음으로 등록할 경우 기본값으로 설정을 한다.
		...
		// receipt_code 값 설정
		$where = !$comp_idx && !$part_idx ? " import_yn = 'Y'":"1";		
		$where = $where . ($param['menu_depth'] > 1 ? " and left(receipt_code, ". strlen($param['receipt_code']) . ") = '" . $param['receipt_code'] ."' " : "");
		//$where = $where .($comp_idx ? " and comp_idx = '" . $comp_idx . "'" : "");
		//$where = $where .($part_idx ? " and part_idx = '" . $part_idx . "'" : "");

        $where .= "and ( comp_idx = '" . $comp_idx . "' ";
        if($comp_info['sc_code']) $where .= "  or sc_code = '" . $comp_info['sc_code'] . "' ";
        if($comp_info['schul_code']) $where .= "  or schul_code = '" . $comp_info['schul_code'] . "' ";
        $where .=")";


		$last_depth = ($param['menu_depth'] > 1 ? (int)$param['menu_depth'] * 2 : 2);
		$data = query_view("select MAX(left(receipt_code, " . $last_depth .")) as max_code from " . $table . " where " . $where);	
		
		if(!$data['max_code'])
		{
			$param['receipt_code'] = "01";
		}
		else if(strlen($data['max_code']) < $last_depth)
		{
			$param['receipt_code'] = $param['receipt_code']."01";
		}
		else
		{
			$param['receipt_code'] = sprintf("%0{$last_depth}d", (int)$data['max_code'] + 1);
		}
	...
	//  업체정보 - 학교 코드 입력
		$where = " and comp.comp_idx = ".$comp_idx;
		$comp_info = company_info_data('view', $where);
		$param["sc_code"] = $comp_info['sc_code'];
		$param["org_code"] = $comp_info['org_code'];
		$param["schul_code"] = $comp_info['schul_code'];
...
// 수정 함수
	...
	// 기본값설정
		if ($param['default_yn'] == 'Y')
		{
			$where = " and code.default_yn = 'Y' and code.code_idx != '" . $code_idx . "'" . (!$comp_idx && !$part_idx ? " and code.import_yn = 'Y'":"");
			$where = $where .($comp_idx ? " and code.comp_idx = '" . $comp_idx . "'" : "");
			$where = $where .($part_idx ? " and code.part_idx = '" . $part_idx . "'" : "");
...
//위 정렬 함수
	function sort_up()
	{
		...
		$sort_where = " and code.menu_depth = '" . $data["menu_depth"] . "' and code.sort < '" . $data["sort"] . "'" . (!$comp_idx && !$part_idx ? " and code.import_yn = 'Y'":"");
		$sort_where = $where .($comp_idx ? " and code.comp_idx = '" . $comp_idx . "'" : "");
		$sort_where = $where .($part_idx ? " and code.part_idx = '" . $part_idx . "'" : "");
...
//아래 정렬 함수
	function sort_down()
	{
		...
		$sort_where = " and code.menu_depth = '" . $data["menu_depth"] . "' and code.sort > '" . $data["sort"] . "'". (!$comp_idx && !$part_idx ? " and import_yn = 'Y'":"");
		$sort_where = $where .($comp_idx ? " and code.comp_idx = '" . $comp_idx . "'" : "");
		$sort_where = $where .($part_idx ? " and code.part_idx = '" . $part_idx . "'" : "");
		//$sort_where = " and code.comp_idx = '" . $comp_idx . "' and code.part_idx = '" . $part_idx . "' and code.menu_depth = '" . $data["menu_depth"] . "' and code.sort > '" . $data["sort"] . "'";
