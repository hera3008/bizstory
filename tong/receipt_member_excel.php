...
// 목록, 페이지관련
	...
	$query_string = "select mem_idx, mem_name from member_info where mem_idx in ('" . $code_member . "')";
    ...
    ->setTitle("담당자별 통계 다운로드")
    ->setSubject("담당자별 통계 다운로드")
    ->setDescription("")
    ->setKeywords("")
	->setCategory("");
	...
	function setPHPExcelData($objPHPExcel, $last_number, $title, $client_infos) {
		$last_column = "";
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '담당자명');
	...
	if ($list_type == 'day') {
		...
		foreach ($client_list as $k => $client_data)
		{
			...
			{
				$array_day[0] = $client_data['mem_name'];
		...
		for ($idx = 0; $idx < $client_count; $idx++) {
			foreach ($list as $k => $data)
			{
				if (is_array($data))
				{
					if ($client_infos[$idx][0] == $data['mem_name']) {
		...
		$filename = $syear . "년 " . $smonth ."월 일별 담당자별 통계";
	} else {
		$client_infos = array();
		$last_month = 12;
		//0으로 초기화
		foreach ($client_list as $k => $client_data)
		{
			if (is_array($client_data))
			{
				$array_month[0] = $client_data['mem_name'];
	...
	//$filename = iconv("UTF-8", "EUC-KR", "담당자별 통계");
