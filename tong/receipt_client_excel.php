...
// 목록, 페이지관련
	...
	if ($code_client != '') {
		if (strpos($code_client, ',')) {
			$code_client = str_replace(",", "','", $code_client);
		}
		$client_where = " and rid.ci_idx in ('" . $code_client . "')";
	}
	if ($list_type == 'day') {
	...
	$query_string = "select ci_idx, client_name from client_info where ci_idx in ('" . $code_client . "')";
	...
	$query_string = "
	select client_name, ci_idx, ymd, count(*) cnt from (
		select
			ci.client_name, rid.ri_idx, rid.ci_idx, rid.receipt_class, " . $day_column . " ymd
		from
			...
			left join client_info ci on ci.del_yn = 'N' and ci.ci_idx = rid.ci_idx
		where
		...
		group by client_name, ci_idx, ymd
    ...
    ->setTitle("거래처별 통계 다운로드")
    ->setSubject("거래처별 통계 다운로드")
    ->setDescription("")
    ->setKeywords("")
	->setCategory("");
	...
	function setPHPExcelData($objPHPExcel, $last_number, $title, $client_infos) {
		$last_column = "";
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '거래처명');
		...
		$objPHPExcel->getActiveSheet()->setTitle('거래처별 통계');
		// 가로 넓이 조정
	...
	if ($list_type == 'day') {
		...
		foreach ($client_list as $k => $client_data)
		{
			...
			{
				$array_day[0] = $client_data['client_name'];
		...
		for ($idx = 0; $idx < $client_count; $idx++) {
			foreach ($list as $k => $data)
			{
				if (is_array($data))
				{
					if ($client_infos[$idx][0] == $data['client_name']) {
		...
		$filename = $syear . "년 " . $smonth ."월 일별 거래처별 통계";
	} else {
		$client_infos = array();
		$last_month = 12;
		//0으로 초기화
		foreach ($client_list as $k => $client_data)
		{
			if (is_array($client_data))
			{
				$array_month[0] = $client_data['client_name'];
    ...
    //$filename = iconv("UTF-8", "EUC-KR", "거래처별 통계");
