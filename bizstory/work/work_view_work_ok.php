...
// 업무 내부저장
	function post_file()
	{
		global $_POST, $_SESSION, $sess_str, $comp_work_path, $comp_work_dir, $tmp_path;
		...
		{
		// 업무 정보
			$wi_where = " and wi.wi_idx = '" . $wi_idx . "'";
			$pro_data = work_info_data('view', $wi_where);
			...
			foreach ($old_list as $old_k => $old_data)
			{
				...
				// 추가 폴더생성
					...
					foreach ($new_folder_arr as $folder_k => $folder_v)
					{
						...
						{
							$new_file_sname  = 'W_' . $comp_idx . '_' . $part_idx . '_' . $pro_idx . '_' . $wi_idx . '_' . time();
