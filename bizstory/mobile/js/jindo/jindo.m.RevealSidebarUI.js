...
jindo.m.RevealSidebarUI = jindo.$Class({
	...
	**/
	_slide : function(isRight, nDuration) {
		if (this._oTransition.isPlaying()) {
			return;
		}
		...
		**/
		if(this.fireEvent("beforeSlide", {
			...
			if(isRight) {
				if(this._htWElement["left"]) { this._htWElement["left"].css("zIndex",5).hide(); }
				if(this._htWElement["right"]) { this._htWElement["right"].css("zIndex",6).show(); }
				self._htWElement["blocker"].css("left","0px");
				this._sStatus = "right";
			} else {
				if(this._htWElement["right"]) { this._htWElement["right"].css("zIndex",5).hide(); }
			...
			this._oTransition.queue( this._htWElement["main"].$value(), nDuration, {
				...
				fCallback : function() {
					...
					if(jindo.m.hasOffsetBug()) {
						...
						htCss[sPrefix + "Transform"] =  "translate" + self.sTranOpen + "0px,0px" + self.sTranEnd;
						htCss["left"] = nPos + "px";
						self._htWElement["main"].css(htCss);
					}
					/**
						페이지가 이동된 후에 발생
		...
		**/
		if(this.fireEvent("beforeRestore", {
			...
			var self=this,
				...
				this._oTransition.queue(this._htWElement["main"].$value(), nDuration, {
				htTransform : {
					"transform" : "translate" + self.sTranOpen + (jindo.m.hasOffsetBug() ? -nPos : 0) + "px,0px" + self.sTranEnd
				},
				fCallback : function() {
					self._sStatus = "main";
					self._htWElement["blocker"].hide();
					if(self._htWElement["left"]) { self._htWElement["left"].hide(); }
					if(self._htWElement["right"]) { self._htWElement["right"].hide(); }
