...
**/
jindo.m.SwapCommon = jindo.$Class({
  /* @lends jindo.m.SwapCommon.prototype */
  /**
      초기화 함수
  ...
  **/  
  ...
  /**
      jindo.m.SwapCommon 에서 사용하는 모든 인스턴스 변수를 초기화한다.
  **/
  _initVar: function() {
    this._htWElement = {};
  ...
  /**
      jindo.m.SwapCommon 에서 사용하는 모든 엘리먼트의 참조를 가져온다.
      @param {Varient} el 엘리먼트를 가리키는 문자열이나, HTML엘리먼트
  **/
  _setWrapperElement: function(el) {
    this._htWElement["view"] = jindo.$Element(el).css({
      "overflow" : "hidden",
  ...
  **/
  ...
  /**
      jindo.m.SwapCommon 에서 사용하는 모든 이벤트를 할당한다.
  **/
  ...
  /**
      jindo.m.SwapCommon 에서 사용하는 모든 이벤트를 해제한다.
  **/
  ...
  **/
  _onStart : function(we) {
    ...
    if(this._startImpl(we)) {
      return;
    }
    this._isStop = false;
    /**
        beforeTouchStart touchStart가 시작되기 전 발생 (jindo.m.Touch의 touchStart 속성과 동일)
        <br/>상세내역은 <auidoc:see content="jindo.m.Touch">[jindo.m.Touch]</auidoc:see>을 참조하기 바란다.
    ...
    **/
    if(this.fireEvent("beforeTouchStart",we)) {    
      /**
          ...
          @param {Function} stop 수행시 touchMove 이벤트가 발생하지 않는다.
      **/
      if(!this.fireEvent("touchStart",we)) {
          we.stop();
      }
    } else {
        we.stop();
  ...
  _onMove : function(we) {
    ...
    **/
    if(this.fireEvent("beforeTouchMove",we)) {
      /** 시스템 스크롤 막기 */
      if(!this._preventSystemEvent(we)) {
        return;
      }
      ...
      /**
          ...
          @param {Function} stop 수행시 touchEnd 이벤트가 발생하지 않는다.
      **/
      if(!this.fireEvent("touchMove",we)) {
          we.stop();
      }
    } else {
        we.stop();
  ...
  **/
  ...
  /**
      jindo.m.SwapCommon 에서 사용하는 모든 객체를 release 시킨다.
      @method destroy
  **/
  destroy: function() {
    var p;
