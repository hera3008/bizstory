...
<script>
    ...
    $(document).ready(function()
    {
		...
		getListOpt('consult', 'my');

    });
    
    function viewIt(idx) {
    	location.href = "consult_view.php?idx=" + idx;
    }
    ...
    function getList() {
    	...
    	//	console.log(json.list_data);				
    					listData += '<li class="barmenu2 loop"><a href="javascript:" onclick="viewIt(' + itemData.cons_idx + ')">'
    							+ '<strong class="date"><span>' + itemData.reg_date + '</span></strong>'
    							+ '<strong class="gubun">[' + itemData.client_name + ']</strong>'
    							+ itemData.subject
    							//+ itemData.important_img
    							+ itemData.total_file_str
    							+ itemData.total_comment_str
    							+ '</a></li>';
    				});
