...
<?
	...
	if ($form_chk == 'Y')
	{
		...
		//$chk_where = " and ri.ri_idx = '" . $ri_idx . "'";
		//$chk_data = receipt_info_data('view', $chk_where);

		//$data = receipt_list_data($ri_idx, $chk_data);
?>
...
?>
				<div class="receipt_area">
					<span class="btn_del fr"><input type="button" value="삭제" onclick="check_delete('<?=$ri_idx;?>')" /></span>
					<span class="btn_big fr"><input type="button" value="수정" onclick="data_form_open('<?=$ri_idx;?>')" /></span>
				</div>
<?
	}
?>
