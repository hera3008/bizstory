...
/*
	생성 : 2012.07.04
	수정 : 2013.05.22
	위치 : 설정폴더(총관리자용) > 컨텐츠관리 > 배너관리 > 사이트배너 - 목록
*/
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				if ($auth_menu['mod'] == "Y")
				{
					...
					$btn_modify = "open_data_form('" . $data['bi_idx'] . "')";
				}
				else
				{
			...
			// 배너이미지
				...
				{
					$img_view = '<img src="' . $banner_dir . '/' . $data['img_sname1'] . '" alt="' . $data['img_sname1'] . '" width="257px" height="125px" />';
				}
