...
//기초값 검사
	...
	//필수검사
		...
		$chk_param['require'][] = array("field"=>"menu_depth", "msg"=>"분류단계");
...
// 등록 함수
	...
	// 기본값설정
		...
		{
			$page_where = " and code.default_yn = 'Y'";
			$page_data = company_class_data("page", $page_where);
	...
	// 등록
		...
		if ($param['viewer_yn'] == '') $param['viewer_yn'] = 'N';
		$param['default_price'] = str_replace(',', '', $param['default_price']);
...
// 수정 함수
	...
	// 기본값설정
		if ($param['default_yn'] == 'Y')
		{
			...
			$page_data = company_class_data("page", $where);
			if ($page_data['total_num'] > 0)
			{
				$str = '{"success_chk" : "N", "error_string" : "설정된 기본값이 있습니다."}';
				echo $str;
	...
	// 변경전의 데이타를 가지고 온다.
		...
		$old_data = company_class_data("view", $where);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	// 상위가 다를경우
		...
		// 자기 하위것들 같이 움직이기
			...
			$down_list = company_class_data("list", $where, "", "", "");
...
//위 정렬 함수
	function sort_up()
	{
		...
		$sort_where = " and code.menu_depth = '" . $data["menu_depth"] . "' and code.sort < '" . $data["sort"] . "'";
		$sort_order = "code.sort desc";
		$prev_data = company_class_data('view', $sort_where, $sort_order);
...
//아래 정렬 함수
	function sort_down()
	{
		...
		$next_data = company_class_data('view', $sort_where, $sort_order);
...
// 메뉴권한 함수
	...
	// 데이타 확인
		$where = " and ccm.code_idx = '" . $code_idx . "' and ccm.mi_idx = '" . $mi_idx . "'";
		$data = company_class_menu_data('page', $where);

	// 데이타가 없을 경우 등록
		if ($data['total_num'] == 0)
		{
			$command    = "insert"; //명령어
			$table      = "company_class_menu"; //테이블명
			$conditions = ""; //조건
		...
		else
		{
			$command    = "update"; //명령어
			$table      = "company_class_menu"; //테이블명
			$conditions = "code_idx = '" . $code_idx . "' and mi_idx = '" . $mi_idx . "'"; //조건
