...
/*
	생성 : 2012.07.04
	위치 : 설정폴더(관리자) > 컨텐츠관리 > 배너관리 > 사이트배너 - 등록/수정
*/
	$banner_type = '2';
	$bi_idx = $idx;
...
// 변수
	...
	$form_chk = 'N';
	if (($auth_menu['int'] == 'Y' && $mem_idx == '') || ($auth_menu['mod'] == 'Y' && $mem_idx != '')) // 등록, 수정권한
	{
		$form_chk = 'Y';
	}
	else
...
<div class="ajax_write">
	<div class="ajax_frame">
		...
		<form id="postform" name="postform" method="post" class="writeform" action="<?=$this_page;?>" onsubmit="return check_form()">
			<?=$form_default;?>
			<input type="hidden" name="upload_fnum" id="upload_fnum" value="1" />
			...
			?>
						<div class="filewrap">
							<div class="file" id="file_fname1_view">
								<?=$img_str;?>
								<a href="javascript:void(0);" class="btn_con" onclick="file_delete_chk('<?=$data['bi_idx'];?>', '1')"><span>삭제</span></a>
							</div>
						</div>
			<?
				}
...
<script type="text/javascript">
//<![CDATA[
	file_setting('file_fname1', 'site_banner', '1', '<?=$file_multi_size;?>', '');
