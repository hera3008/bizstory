...
/*
	생성 : 2012.05.24
	수정 : 2012.07.04
	위치 : 설정폴더(관리자) > 컨텐츠관리 > 배너관리 > 에이전트배너 - 실행
*/
...
//등록 함수
	...
	// 총 저장 파일
		...
		{
			$upfile_data = upload_file_save($i, $tmp_path, $data_path, $_POST, $bi_idx, 'banner');

			if ($upfile_data[$i]['f_name'] != '')
			{
				$param['img_fname' . $j] = $upfile_data[$i]['f_name'];
				$param['img_sname' . $j] = $upfile_data[$i]['s_name'];
				$param['img_size' . $j]  = $upfile_data[$i]['f_size'];
				$param['img_type' . $j]  = $upfile_data[$i]['f_type'];
				$param['img_ext' . $j]   = $upfile_data[$i]['f_ext'];
				$j++;
			}
		}
