...
//------------------------------------ 인쇄 열기
	function open_list_print(idx, print_url)
	{
		...
		f.target = 'print';
		f.action = print_url;
		window.open('', 'print', 'width=1000, height=600, toolbar=no, top=30, left=30, resizable=yes, scrollbars=yes');
		f.submit();
	}
...
//------------------------------------ 다운로드문서
	function open_data_down(idx, down_url)
	{
		...
		f.target = 'down';
		f.action = down_url;
		window.open('', 'down', 'width=1000, height=600, toolbar=no, top=30, left=30, resizable=yes, scrollbars=yes');
		f.submit();
	}
...
/*
		...
		$('#' + str).click(function(e) {
			var difWidth = maskWidth - 400;

			divTop = e.clientY + 20;

			var clickLeft = e.clientX - 20;
			if (difWidth < clickLeft) divLeft = difWidth;
			else divLeft = clickLeft;
			alert(divTop);
		});
		...
		$.ajax({
			...
			success: function(msg) {
				...
				$("#backgroundPopup").css({"opacity": "0.7",'width':'100%','height':maskHeight}).fadeIn("slow");
				$("#data_form").slideDown("slow");
				$(".popupform").css({"top": divTop, "left": divLeft, "width": "350px"});
				$("#data_form").html(msg);
			}
		});
...
//------------------------------------ 선택인쇄
	function list_print_detail()
	{
		...
		window.open('', 'print', 'width=1000, height=700, toolbar=no, top=30, left=30, resizable=yes, scrollbars=yes');
		f.submit();
	}
