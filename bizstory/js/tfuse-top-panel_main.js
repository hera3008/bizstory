...
{
	var speed = 400;
	var favusetopanelshow = getCookie('favusetopanelshow');
	var favuse_top_panel  = jQuery("#tfuse-top-sliding-panel-container");
	//jQuery("#bookmark").prepend(favuse_top_panel);
	favuse_top_panel.show();

	// slideup on page load
	if(favusetopanelshow == 'yes')
	{
		jQuery("#tfuse-top-sliding-panel-container").css({top:"-500px"});
	...
	else
	{
		...
		jQuery("#tfuse-top-sliding-panel-container").animate({top:"-500px"}, 900);
		jQuery(this).removeClass("ajax-close-tfuse-top-slide").addClass("ajax-open-tfuse-top-slide");

		document.cookie = "favusetopanelshow=yes; path=/";
	}
	...
	jQuery("#tfuse-top-sliding-panel-container #tfuse-top-sliding-panel-btn-slide").click(function()
	{
		...
		{
			jQuery("#tfuse-top-sliding-panel-container").animate({top:"-500px"}, speed);
			jQuery(this).removeClass("ajax-close-tfuse-top-slide").addClass("ajax-open-tfuse-top-slide");
		}
	});
