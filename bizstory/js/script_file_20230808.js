...
//------------------------------------ 업로드파일설정
	...
	{	
		...
		if(!sort || sort == '' || sort == undefined || sort == 0){
			sort = 100;
			upload_file_sort[file_name] = 100 ;
		}else{
		...
		$('#'+input_upload_name).css('display', 'none');
		...
		if(sort == 1){
			var html = "<style>.fileDrop{border:1px #ccc solid;margin:5px 0px;padding:0;min-height: 23px;width: 50%; background-color:#fff;}</style>";
				...
				html += "<div style='padding-right: 5px;'><a href='javascript:void(0);' id='btn_file_select_" + file_name+ "' class='btn_big_green'><span>파일업로드</span></a></div>";
				html += "<div id='fileDrop_" + file_name+ "' data='" + file_name+ "' class='fileDrop'></div>";
				html += "</div>";
		}else{
			var html = "<style>.mfileDrop{border:1px #ccc solid;margin:5px 0px;padding:0;min-height: 80px;width: 100%; background-color:#fff;} .mfileDrop #file_drop_tip{text-align: center;margin-top: 25px;font-size: 12px;}</style>";
				html += "<div id='fileDrop_" + file_name+ "' class='mfileDrop' data='" + file_name+ "'><p id='file_drop_tip'>첨부하실 파일을 여기에 끌어 놓으세요.</p></div>";
				html += "<a href='javascript:void(0);' id='btn_file_select_" + file_name+ "' class='btn_big_green'><span>파일업로드</span></a>";
		}

		$('#'+input_upload_name).after(html);
		
		$('#btn_file_select_'+input_upload_name).on('click',function (e) {
			e.preventDefault();
			$('#'+input_upload_name).click();
		});

		
		$('#'+input_upload_name).on('change', function(){
			 select_file($('#'+input_upload_name)[0].files); 
			 file_name = $(this).attr('id');
		});
		
		if(sort > 1){
			file_drop_down(input_upload_name);
		}
    ...
    function select_file(file_object){
        ...
        if(files != null){
            ...
            for(var i = 0; i < files.length; i++){
				/*
				if(upload_file_total >= upload_file_sort[file_name]){
					$("#loading").fadeOut('slow');
                    check_auth_popup("총 업로드 가능한 파일 " + upload_file_sort[file_name] +"개 입니다.");
                    break;
				}*/
    ...
    function add_file_list(fIndex, file_info){
		...
		else
		{
			...
			if(upload_file_sort[file_name] == 1){
				...
				if($("input[name='"+fup_name+"_save_name']").length > 0){
			...
			setTimeout( () => {
				var html = "";
					html += "<div id='fileTr_" + file_name +"_"+ trIndex + "' style='padding-left: 5px;display: contents;'>";
