...
//기초값 검사
	...
	{
	//필수검사
		$chk_param['require'][] = array("field"=>"code_name", "msg"=>"근태상태명");
...
// 등록 함수
	...
	// 기본값설정
		if ($param['default_yn'] == 'Y')
		{
			...
			$page_data = code_dili_status_data("page", $where);
	...
	// 등록
		...
		if ($param['request_yn'] == '') $param['request_yn'] = 'N';
		...
		$data = query_view("
			...
			where
				del_yn = 'N'
				and comp_idx = '" . $comp_idx . "' and part_idx = '" . $part_idx . "'");
...
// 수정 함수
	...
	// 수정
		...
		if ($param['default_yn'] == '') $param['default_yn'] = 'N';
		if ($param['request_yn'] == '') $param['request_yn'] = '0';
...
//위 정렬 함수
	function sort_up()
	{
		...
		$prev_data = code_dili_status_data('view', $sort_where, $sort_order);
...
//아래 정렬 함수
	function sort_down()
	{
		...
		$next_data = code_dili_status_data('view', $sort_where, $sort_order);
