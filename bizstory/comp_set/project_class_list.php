...
// 변수
	...
	//$list = code_project_class_data('list', $where, "concat(up_code_idx, concat(',', code_idx)) asc, sort asc", '', '');
	$list = code_project_class_data('list', $where, " code.sort asc", '', '');
...
<table class="tinytable">
	...
	<thead>
		<tr>
			...
			<th class="nosort"><h3>프로젝트분류명</h3></th>
	...
	else
	{
        ...
        foreach($list as $k => $data)
        {
            if (is_array($data))
            {
                ...
                $up_code_idx   = $data['up_code_idx'];
                $sort       = $data['sort'];
                
                $sort_chk[$menu_depth][$up_code_idx][$sort] = $sort;
            }
        }
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				$sort_data = query_view("select min(sort) as min_sort, max(sort) as max_sort from code_project_class where del_yn = 'N' and comp_idx ='" . $code_comp . "' and part_idx ='" . $code_part . "'");
...
?>
			...
			<td><div class="left depth_<?=$data["menu_depth"];?>"><?=$code_name;?></div></td>
