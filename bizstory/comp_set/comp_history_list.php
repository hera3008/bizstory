...
// 변수
	...
	$where = " and ch.comp_idx = '" . $code_comp . "'";
	$list = company_history_data('list', $where, '', '', '');
...
<table class="tinytable">
	...
	<thead>
		<tr>
			...
			<th class="nosort"><h3>년</h3></th>
			<th class="nosort"><h3>월</h3></th>
			<th class="nosort"><h3>일</h3></th>
	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				$sort_data = query_view("select min(sort) as min_sort, max(sort) as max_sort from company_history where del_yn = 'N' and comp_idx ='" . $code_comp . "'");

				if ($auth_menu['mod'] == "Y")
				{
					$btn_up     = "check_code_data('sort_up', '', '" . $ch_idx . "', '')";
					$btn_down   = "check_code_data('sort_down', '', '" . $ch_idx . "', '')";
					$btn_view   = "check_code_data('check_yn', 'view_yn', '" . $ch_idx . "', '" . $data["view_yn"] . "')";
					$btn_modify = "popupform_open('" . $ch_idx . "')";
				}
				else
				{
				...
				if ($auth_menu['del'] == "Y") $btn_delete = "check_delete('" . $ch_idx . "')";
