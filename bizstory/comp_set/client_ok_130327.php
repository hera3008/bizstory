...
// 등록 함수
	function post()
	{
		...
		$data = query_view("select max(sort) as sort from " . $table . " where del_yn = 'N' and comp_idx = '" . $comp_idx . "'");
		$param['sort'] = ($data['sort'] == "") ? '1' : $data['sort'] + 1;
	...
	// 거래처코드구하기
		$client_code_where = " and cci.comp_idx = '" . $comp_idx . "' and cci.ci_idx = 0";
		$client_code_data = client_code_info_data('view', $client_code_where);

		$param['client_code'] = $client_code_data['client_code'];
