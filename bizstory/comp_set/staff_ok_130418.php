...
// 등록 함수
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// V-Drive/Member -> 추가한 직원폴더생성
		$mem_idx  = $param['mem_idx'];
		$mem_name = $param['mem_name'];

	// 상위값
		$code_where = " and fi.part_idx = '" . $part_idx . "' and fi.dir_file = 'folder' and fi.file_name = 'Member' and fi.file_path = '/V-Drive'";
		$code_data = filecenter_info_data('view', $code_where);
		$up_fi_idx     = $code_data['fi_idx'];
		$up_file_path  = $code_data['file_path'];
		$up_file_rpath = $code_data['file_rpath'];
		$up_file_name  = $code_data['file_name'];
		$up_file_sname = $code_data['file_sname'];

	// 폴더등록
		$chk_data = query_view("select max(fi_idx) as fi_idx from filecenter_info");
		$fi_idx = ($chk_data['fi_idx'] == '') ? '1' : $chk_data['fi_idx'] + 1;
		$file_path  = $code_data['file_path'] . '/' . $code_data['file_name'];
		$file_rpath = $code_data['file_rpath'] . '/' . $code_data['file_sname'];
		$file_name  = $mem_name;
		$file_sname = $mem_name;

		$file_query = "
			insert into filecenter_info set
				fi_idx     = '" . $fi_idx . "',
				comp_idx   = '" . $comp_idx . "',
				part_idx   = '" . $part_idx . "',
				dir_file   = 'folder',
				...
				up_fi_idx  = '" . $up_fi_idx . "',
				file_path  = '" . $file_path . "',
				file_rpath = '" . $file_rpath . "',
				file_name  = '" . $file_name . "',
				file_sname = '" . $file_sname . "',
				set_type = 'fix',
		...
		";
		db_query($file_query);
		query_history($file_query, 'filecenter_info', 'insert', $comp_idx, $part_idx, $mem_idx);

	// 이력등록
		$history_query = "
			insert into filecenter_history set
				fi_idx       = '" . $fi_idx . "',
				new_subject  = '" . $file_name . "',
				file_sname   = '" . $file_sname . "',
				history_memo = '" . $file_name . " 폴더를 생성했습니다.',
				comp_idx = '" . $comp_idx . "',
				part_idx = '" . $part_idx . "',
				dir_file = 'folder',
				reg_type = 'insert',
		...
		query_history($history_query, 'filecenter_history', 'insert', $comp_idx, $part_idx, $mem_idx);

	// 권한등록
		$quth_query = "
			insert into filecenter_auth set
				fi_idx   = '" . $fi_idx . "',
				dir_view  = '1', dir_read  = '1', dir_write = '1', mem_idx  = '" . $mem_idx . "',
				comp_idx = '" . $comp_idx . "',
				part_idx = '" . $part_idx . "',
				reg_id   = '" . $mem_idx . "',
				reg_date = '" . time() . "',
		...
		";
		db_query($quth_query);
		query_history($quth_query, 'filecenter_auth', 'insert', $comp_idx, $part_idx, $mem_idx);
