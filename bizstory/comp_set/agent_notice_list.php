...
// 변수
	...
	$where = " and an.comp_idx = '" . $code_comp . "' and an.part_idx = '" . $code_part . "'";
	$list = agent_notice_data('list', $where, '', '', '');
	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				if ($auth_menu['mod'] == "Y")
				{
					$btn_up     = "check_code_data('sort_up', '', '" . $data['an_idx'] . "', '')";
					$btn_down   = "check_code_data('sort_down', '', '" . $data['an_idx'] . "', '')";
					$btn_view   = "check_code_data('check_yn', 'view_yn', '" . $data['an_idx'] . "', '" . $data["view_yn"] . "')";
					$btn_modify = "popupform_open('" . $data['an_idx'] . "')";
				}
				else
				{
				...
				if ($auth_menu['del'] == "Y") $btn_delete = "check_delete('" . $data['an_idx'] . "')";
				else $btn_delete = "check_auth_popup('delete');";

				if ($data['link_url'] == '') $content = $data['content'];
				else $content = '<a href="http://' . $data['link_url'] . '" target="_blank">' . $data['content'] . '</a>';
?>
	<tr>
		<td>
			<div class="sort">
...
?>
		...
		<td><div class="left"><?=$data["link_url"];?></div></td>
