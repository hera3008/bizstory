...
// 변수
	...
	if ($form_chk == 'Y')
	{
		...
		$data = code_report_class_data("view", $where);
...
<div class="ajax_write">
	...
	<div class="ajax_frame">
		...
		<form id="postform" name="postform" method="post" class="writeform" action="<?=$this_page;?>" onsubmit="return check_form()">
			<?=$form_all;?>
			<fieldset>
				<legend class="blind">점검항목 폼</legend>
				<table class="tinytable write" summary="점검항목 등록/수정합니다.">
				...
				<tbody>
					...
					<tr>
						<th><label for="post_code_name">점검항목명</label></th>
						<td>
							<div class="left">
								<input type="text" name="param[code_name]" id="post_code_name" value="<?=$data['code_name'];?>" size="80" title="점검항목명을 입력하세요." class="type_text" />
							</div>
						</td>
					</tr>
					<tr>
						<th><label for="post_input_type">입력형태</label></th>
						<td>
							<div class="left">
								<?=code_select($set_input_type, 'param[input_type]', 'post_input_type', $data['input_type'], '입력형태', '입력형태선택', '', '', '');?>
							</div>
						</td>
					</tr>
					<tr>
						<th><label for="post_input_value">입력값</label></th>
						<td>
							<div class="left">
								<input type="text" name="param[input_value]" id="post_input_value" value="<?=$data['input_value'];?>" size="50" title="입력값을 입력하세요." class="type_text" />
								* 라디오버튼, 체크박스, 셀렉트박스일 경우에 해당됩니다. 값과의 구분은 , 으로 합니다.
							</div>
						</td>
					</tr>
					<tr>
						<th><label for="post_input_default">입력기본값</label></th>
						<td>
							<div class="left">
								<input type="text" name="param[input_default]" id="post_input_default" value="<?=$data['input_default'];?>" size="30" title="입력기본값을 입력하세요." class="type_text" />
							</div>
						</td>
					</tr>
					<tr>
