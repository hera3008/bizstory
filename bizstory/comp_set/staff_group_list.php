...
// 변수
	...
	$where = " and csg.comp_idx = '" . $code_comp . "' and csg.part_idx = '" . $code_part . "'";
	$list = company_staff_group_data('list', $where, '', '', '');
...
<table class="tinytable">
	...
	<thead>
		<tr>
			...
			<th class="nosort"><h3>직원그룹명</h3></th>
...
<?
	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				$csg_idx = $data['csg_idx'];
				$sort_data = query_view("select min(sort) as min_sort, max(sort) as max_sort from company_staff_group where del_yn = 'N' and comp_idx ='" . $code_comp . "' and part_idx ='" . $code_part . "'");

				if ($auth_menu['mod'] == "Y")
				{
					$btn_up      = "check_code_data('sort_up', '', '" . $csg_idx . "', '')";
					$btn_down    = "check_code_data('sort_down', '', '" . $csg_idx . "', '')";
					$btn_view    = "check_code_data('check_yn', 'view_yn', '" . $csg_idx . "', '" . $data["view_yn"] . "')";
					$btn_default = "check_code_data('check_yn', 'default_yn', '" . $csg_idx . "', '" . $data["default_yn"] . "')";
					$btn_modify  = "popupform_open('" . $csg_idx . "')";
				}
				else
				{
				...
				if ($auth_menu['del'] == "Y") $btn_delete = "check_delete('" . $csg_idx . "')";
