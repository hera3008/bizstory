...
// 등록 함수
	function post()
	{
		...
		if ($param["client_all"] == "") $param["client_all"] = "N";
		if ($param["import_type"] == '') $param["import_type"] = '0';

		$chk_data = query_view("select max(abn_idx) as abn_idx, max(order_idx) as order_idx from " . $table);
		$param['abn_idx']   = ($chk_data['abn_idx'] == '') ? '1' : $chk_data['abn_idx'] + 1;
		$param['order_idx'] = ($chk_data['order_idx'] == '') ? '1' : $chk_data['order_idx'] + 1;

		if ($param['ccg_idx'] > 0) $param["client_all"] = "N";
	...
	// 파일저장
		...
		$data_path1 = $comp_bnotice_path;
		files_dir($data_path1);

		$data_path = $data_path1 . '/' . $abn_idx;
		files_dir($data_path);
	...
	// 총 저장 파일
		...
		for ($i = 1; $i <= 2; $i++)
		{
			$upfile_data = upload_file_save($i, $tmp_path, $data_path, $_POST, $abn_idx, 'agent_bnotice', 'f_fname');
			...
			// 데이타 확인
				...
				else
				{
					$query_update = "update " . $file_table . " set
						...
						where
							del_yn = 'N' and comp_idx = '" . $comp_idx . "' and abn_idx = '" . $abn_idx . "' and sort ='" . $i . "'";
...
// 수정 함수
	...
	// 총 저장 파일
		...
		for ($i = 1; $i <= 2; $i++)
		{
			$upfile_data = upload_file_save($i, $tmp_path, $comp_bnotice_path, $_POST, $abn_idx, 'agent_bnotice', 'f_fname');
...
// 파일삭제 함수
	function file_delete()
	{
		...
		if ($img_sname != "") @unlink($comp_bnotice_path . '/' . $data['abn_idx'] . '/' . $img_sname);
