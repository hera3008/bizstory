	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				$sort_data = query_view("select min(sort) as min_sort, max(sort) as max_sort from code_receipt_class where del_yn = 'N' and comp_idx ='" . $code_comp . "' and part_idx ='" . $code_part . "' and up_code_idx = '" . $data["up_code_idx"] . "'");
