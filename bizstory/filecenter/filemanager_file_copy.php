...
<script src="<?=$local_dir;?>/common/js/fileTree/jqueryFileTree.js?t=<?=time()?>" type="text/javascript"></script>
...
<div class="ajax_write">
	...
	<div class="ajax_frame">
		...
		?>
					...
					<div class="section">
						<div class="fr">
							<span class="btn_big_green"><input type="button" value="<?=$file_title;?>" onclick="check_copy_move_file()" /></span>
	...
	function check_copy_move()
	{
		...
		if (dir_write_auth == 'N') {
			...
			else
			{
				...
				$.ajax({
					type: 'post', dataType: 'jsonp', url: '<?=$set_filecneter_url;?>/file_ok.php', jsonp : 'callback',
					data: $('#copyform').serialize(),
					success: function(msg) {
...
//------------------------------------ 파일위치변경
    ...
    function open_dir_change2(up_idx, up_level, up_type)
    {
        $('#list_up_idx').val(up_idx);
        $('#list_up_level').val(up_level);
        $('#copy_up_idx').val(up_idx);
        ...
        $("#loading").fadeIn('slow');
        $.ajax({
            type:'post', dataType: 'json', url: '<?=$local_dir?>/bizstory/filecenter/ajax/filecenter_change_check.php',
            data: $('#listform').serialize(),
            success: function(data) {
                
                if (data.navi_path != null) {
                    $('.upload_l2 .change_title').find('strong').html('변경할 위치 : ' + data.navi_path + ' ' + (data.navi_subject == null ? '' : data.navi_subject));
                }
                
                dir_write_auth = data.dir_write_auth;
