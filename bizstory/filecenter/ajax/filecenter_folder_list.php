            ...
            foreach ($info_list as $info_k => $info_data)
            {
                if (is_array($info_data))
                {
                    ...
                    if ($ul_id_str == 'ffsubmenu')
                    {
                        ...
                        $dirs[] = array('type'=>'dir', 'params'=>$comp_idx . '||' . $mem_idx . '||' . $next_depth . '||' . $fi_idx . '||' . $pro_end . '||' . $ul_id_str, 'file'=>$file_name);
                    }
                    else
                    {
                        $btn_click = "file_list_view('" . $fi_idx . "', '" . $next_depth . "');";
                        $fi[] = array('type'=>'dir', 'params'=>$comp_idx . '||' . $mem_idx . '||' . $next_depth . '||' . $fi_idx . '||' . $pro_end . '||' . $ul_id_str, 'file'=>$file_name);
                    }
