...
/*
	생성 : 2012.05.25
	위치 : 업무폴더 > 나의업무 > 업무 - 상태 - 요청대기에서 업무완료
*/
...
<div class="pstatus_box">
	<div class="pstatus_top">
		<p class="count">해당업무를 완료하시겠습니까?</p>
	</div>

	<form id="status30_90form" name="status30_90form" action="<?=$this_page;?>" method="post">
		<input type="hidden" id="status30_90_comp_idx" name="comp_idx" value="<?=$code_comp;?>" />
		<input type="hidden" id="status30_90_part_idx" name="part_idx" value="<?=$code_part;?>" />
		<input type="hidden" id="status30_90_wi_idx"   name="wi_idx"   value="<?=$wi_idx;?>" />
		<input type="hidden" id="status30_90_sub_type" name="sub_type" value="status30_90" />
	...
	function form_workstatus()
	{
		...
		if (action_num == 0)
		{
			$.ajax({
				...
				data: $('#status30_90form').serialize(),
				success: function(msg) {
					if (msg.success_chk == "Y")
					{
						popup_work_close();
						list_data();
						view_open($('#status30_90_wi_idx').val());
					}
