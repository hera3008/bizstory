...
<div class="pstatus_box">
	<div class="pstatus_top">
		<p class="count">요청완료를 반려하시겠습니까?</p>
	</div>

	<form id="status30_70form" name="status30_70form" action="<?=$this_page;?>" method="post">
		<input type="hidden" id="status30_70_comp_idx" name="comp_idx" value="<?=$code_comp;?>" />
		<input type="hidden" id="status30_70_part_idx" name="part_idx" value="<?=$code_part;?>" />
		<input type="hidden" id="status30_70_wi_idx"   name="wi_idx"   value="<?=$wi_idx;?>" />
		<input type="hidden" id="status30_70_sub_type" name="sub_type" value="status30_70" />
		...
		<div class="pstatus">
			...
			<div class="pstatus_wrap">
				<div class="pstatus_data">
					<div class="user_edit">
						요청완료를 반려하면, 해당 업무의 담당자에게 반려사유를 알려드립니다.
					</div>
				</div>
			</div>
	...
	function form_workstatus()
	{
		...
		if (action_num == 0)
		{
			$.ajax({
				...
				data: $('#status30_70form').serialize(),
				success: function(msg) {
					if (msg.success_chk == "Y")
					{
						popup_work_close();
						list_data();
						view_open($('#status30_70_wi_idx').val());
					}
