<?
	...
	$bs_data = bbs_setting_data('view', $bs_where);

	$where = " and bc.bs_idx = '" . $bs_idx . "'";
	$orderby = "bc.sort asc";
	$list = bbs_category_data('list', $where, $orderby, '', '');

	$cate_list = $local_dir . '/bizstory/bbs/bbs_cate.php';
	$cate_ok   = $local_dir . '/bizstory/bbs/bbs_cate_ok.php';
...
<div class="ajax_write">
	<div class="ajax_frame">
		...
		<div class="info_frame">
			<span><strong><?=$bs_data['subject'];?></strong> 말머리관리</span>
		</div>
...
?>
				<tr>
					...
					<td>
						<div class="left">
							<input type="text" name="post_menu_name" id="post_menu_name" class="type_text" title="말머리 입력하세요." />
						</div>
					</td>
		...
		foreach($list as $k => $data)
		{
			if (is_array($data))
			{
				...
				if ($bc_idx == $data['bc_idx'])
				{
					...
					$cate_data = bbs_category_data('view', $cate_where);
?>
				<tr>
					<td>&nbsp;</td>
