...
//입력처리 함수
	...
	// 답변글일 경우
		...
		bbs_link_insert($bs_idx, $param['b_idx'], $_POST, $set_bbs); // 링크등록
	...
	// 공지글일 경우
		...
		if ($notice_yn == "Y")
		{
			$notice_data = query_view("select max(sort) as notice_sort from bbs_notice where del_yn = 'N'");
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// 파일저장
		global $tmp_path, $comp_bbs_path;

		$b_idx = $param['b_idx'];

		$data_path2 = $comp_bbs_path;
	...
	function modify()
	{
		...
		bbs_link_insert($bs_idx, $b_idx, $_POST, $set_bbs); // 링크등록
	...
	// 기존글 확인
		...
		$notice_data = bbs_notice_data("view", $notice_where);
...
//선택복사, 이동
	...
	// 이동, 복사할 게시판설정
		...
		$set_new_bbs = bbs_setting_data("view", $set_where);
		...
		foreach ($chk_b_idx as $k => $b_idx)
		{
			...
			// 이동일 경우 - bs_idx, comp_idx, part_idx 변경
				if ($sub_action == "move_yn")
				{
					...
					";
					db_query($up_query);
					query_history($up_query, 'bbs_info', 'update');
				...
				// 파일
					...
					if ($file_list["total_num"] > 0)
					{
						...
						foreach($file_list as $file_k => $file_data)
						{
							...
							{
								$old_file_path = $comp_bbs_path . '/' . $bs_idx . '/' . $file_data['b_idx'] . '/' . $file_data['img_sname'];
								$new_file_path = $comp_bbs_path . '/' . $select_bs_idx . '/' . $new_b_idx . '/' . $file_data["img_sname"];
								...
								";
								db_query($file_sql);
								query_history($file_sql, 'bbs_file', 'update');
					...
					if ($link_list["total_num"] > 0)
					{
						foreach($link_list as $link_k => $link_data)
						{
							if (is_array($link_data))
							{
								...
								";
								db_query($link_sql);
								query_history($link_sql, 'bbs_link', 'update');
							}
						}
					}
				}
				else // 복사일 경우
				{
					...
					$bbs_data = bbs_info_data("view", $where, "", "", "");
					...
					if ($file_list["total_num"] > 0)
					{
						...
						foreach($file_list as $file_k => $file_data)
						{
							if (is_array($file_data))
							{
								...
								";
								db_query($file_sql);
								query_history($file_sql, 'bbs_file', 'insert');
					...
					if ($link_list["total_num"] > 0)
					{
						foreach($link_list as $link_k => $link_data)
						{
							if (is_array($link_data))
							{
								...
								";
								db_query($link_sql);
								query_history($link_sql, 'bbs_link', 'insert');
							}
						}
					}
				}
