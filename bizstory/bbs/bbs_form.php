...
/////////
	if ($form_chk == 'Y')
	{
		...
		// 파일
			$file_query = "select max(sort) as sort from bbs_file where bs_idx = '" . $bs_idx . "' and b_idx = '" . $b_idx . "'";
			$file_chk = query_view($file_query);
			$file_sort = ($file_chk['sort'] == '') ? '0' : $file_chk['sort'];

			$file_upload_num = $file_chk['sort'];
			$file_chk_num    = ($file_upload_num?$file_upload_num:0) + 1;
		}
		else 
		{
			$file_list = array();
	...
	<div class="ajax_frame">
		...
		?>
				...
				?>
										<li id="file_fname<?=$file_chk;?>_view" class="org_file">
											...
											<a href="javascript:void(0);" class="btn_con" onclick="file_form_delete('<?=$file_data['bf_idx'];?>', '<?=$file_chk;?>')"><span>삭제</span></a>
										</li>
				<?
						}
					}
		...
		?>
								<div class="left">
									...
									<a href="javascript:void(0)" onclick="add_link()" class="file_plus" title="링크추가">추가</a>
								</div>
...
// 에디터관련
	...
	file_setting('file_fname', 'bbs', '', '<?=$file_multi_size;?>', '');
