...
// 링크, 버튼
	...
	//{
	//	$link_form = $local_dir . "/bizstory/project/project_form_activex.php"; // 파일업로드-active
	//}
	//else
	//{
		$link_form = $local_dir . "/bizstory/project/project_form_html.php";    // 파일업로드-html5
	//}
...
?>
<script type="text/javascript" src="<?=$local_dir;?>/bizstory/js/hashchange.js"></script>
<script type="text/javascript">
    $(function() {
        $(window).hashchange(function() {
            ...
            //페이지 이동 시 열려있던 프로젝트 창이 닫히게 하려했으나
            //직접호출 시에도 (로딩이 두번 호출되어 p1420783165 식의 값이 붙어) 상세창이 닫혀
            // 일단 주석처리함 (2015-01-09  고동원)
            /*
            if(hash_info.indexOf('v') >= 0)
            {
                
            }
            else if(hash_info.indexOf('p') >= 0)
            {
                view_close();
            }
            else
            {
                //document.location.hash = "#<%=request.getParameter("page")%>";
            }
            */
        });
    ...
    function select_smember(type, target) {
        $.ajax({
            type: "post", dataType: 'html', url: '<?=$local_dir;?>/bizstory/project/select_member.php',
            data: {
                type: type,                    
                val: $("#search_s" + target).val(),
                target: target
            },
...
<div class="tablewrapper">
	<div id="tableheader">
		...
		<form id="searchform" name="searchform" method="post" action="<?=$this_page;?>" onsubmit="return check_search();">
			...
			<div class="search">
				<p>검&nbsp;&nbsp;색</p>
				<span><label for="search_sproject"><input type="checkbox" id="search_sproject" name="sproject" value="Y" <?=checked('Y', $sproject);?> />완료 프로젝트 보기</label></span>

				<div class="s_area">
				    <a href="javascript:void(0);" class="btn_sml" onclick="select_smember('multi', 'charge')"><span>담당자 선택</span></a>
				    <a href="javascript:void(0);" class="btn_sml" onclick="select_smember('multi', 'apply')"><span>책임자 선택</span></a>
				    <a href="javascript:void(0);" class="btn_sml" onclick="select_smember('multi', 'reg')"><span>등록자 선택</span></a>
				    <!--
					...
					<input type="text" id="search_apply" name="apply" class="type_text" value="<?=$search_apply;?>" title="<?=$search_apply;?>" onblur="if (this.value == '') {this.value = this.title;}" onfocus="if (this.value == this.title) {this.value = '';}" />
					<input type="text" id="search_reg" name="reg" class="type_text" value="<?=$search_reg;?>" title="<?=$search_reg;?>" onblur="if (this.value == '') {this.value = this.title;}" onfocus="if (this.value == this.title) {this.value = '';}" />
                    -->
					...
					<?=$btn_print;?>
					<?=$btn_print_sel;?>
					<div id="charge_member" class="type_text" style="margin-top:3px"></div>
					<div id="apply_member" class="type_text" style="margin-top:3px"></div>
					<div id="reg_member" class="type_text" style="margin-top:3px"></div>
				</div>
			</div>
		</form>
	</div>
	...
	<form id="listform" name="listform" method="post" action="<?=$this_page;?>">
		...
		<input type="hidden" id="search_scharge" name="scharge" value="<?=$search_scharge?>" />
        <input type="hidden" id="search_sapply" name="sapply" value="<?=$search_sapply?>" />
        <input type="hidden" id="search_sreg" name="sreg" value="<?=$search_sreg?>" />
		<?=$form_page;?>
...
//------------------------------------ 삭제시
	function project_delete(pro_idx)
	{
		if (confirm("선택하신 프로젝트를 삭제하시겠습니까? \n\n삭제하시면 파일도 같이 삭제가 됩니다."))
		{
			$("#loading").fadeIn('slow');
