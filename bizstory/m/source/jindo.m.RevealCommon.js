...
jindo.m.RevealCommon = jindo.$Class({
    ...
    **/
    ...
    $init : function(htOption){
        this.option({
            "sClassPrefix" : "reveal-",
            "sDirection" : "right",  // left,right, top, bottom
            "nDuration" : 500,
   ...
   _initVar: function(bInit) {
        ...
        this._nWidth = this._htSize.width;  // 브라우저의 넓이 
        ...
        **/
        ...
        if(htPosInfo.nMarginLeftPos && (this.option("sDirection") == "left" || this.option("sDirection") == "right") && this._bShow){
            if(!this.option("bUseOffsetBug")){
                ...
                this._htWElement["nav"].css("left", (this.option("sDirection") == "left" ? this.option("nMargin") : 0));
...
//         
        // if(!this._bShow && this.option("sDirection") == "left" && this.option("bUseScroll")){
             // if(this._htInstance["scroll"]){
                // this._htInstance["scroll"].height(jindo.$Document().clientSize());
            // }else{
                // console.log(jindo.$Document().clientSize());
                // this._htInstance["scroll"] = new jindo.m.Scroll(this._htWElement["nav"].$value(), {
                    // bUseHScroll : false,
                    // bUseVScroll : true,
                    // bUseMomentum : true,
                    // nDeceleration : 0.0005,
                    // nHeight : jindo.$Document().clientSize()
                // });
            // }
        // }
    ...
    _onDeactivate : function(){
        ...
        this._bShow = this._sCssPrefix = this._htEvent = this._htSize = this._nNavHeight = this._nWidth = null;
