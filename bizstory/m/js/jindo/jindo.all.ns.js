 ...
 * @type mobile
 * @version nightly
 * NHN_Library:Jindo-nightly;JavaScript Framework;
 */
...
jindo.$Jindo.isNumeric = function(nNum){
	return !isNaN(parseFloat(nNum)) && isFinite( nNum );
};
/**

 * 파라메터가 Boolean인지 확인하는 함수.
...
jindo.$Jindo.checkVarType._throwException = function(aArgs, oRules, sFuncName) {
	...
	if (/(\$\w+)(#\w+)?/.test(sFuncName)) {
		sURL = 'http://jindo.nhncorp.com/docs/jindo/archive/Jindo2-nightly/desktop/ko/symbols/' + encodeURIComponent(RegExp.$1) + '.html' + RegExp.$2;
	}
 ...
 * @param {Object} oTypeLists 타입 규칙을 담은 객체
 * @return {$Jindo}
 * @ignore
 *
 ...
 * @param {Element|String} [elBaseElement] 탐색 대상이 되는 DOM 요소. 지정한 요소의 하위 노드에서만 객체를 탐색한다. 생략될 경우 문서를 대상으로 찾는다. id을 넣을 수 있다. 
...
//	}
	...
	cssquery = function(sQuery, oParent, oOptions) {
		...
		var oldID, oOldParent;
		
		if(_isNonStandardQueryButNotException(sQuery)){
			throw new jindo.$Error(jindo.$Except.NOT_SUPPORT_SELECTOR,(oOptions&&oOptions.single?"<sctatic> cssquery.getSingle":"cssquery"));
		}
		if(nNodeType===9){
		}else if(jindo.$Element._contain(document,oParent)){
			if(oParent.id){ oldID = oParent.id;}
			oParent.setAttribute("id",('C' + new Date().getTime() + Math.floor(Math.random() * 1000000)));
			sQuery = _commaRevise("#"+oParent.id+" "+sQuery,", #"+oParent.id);
			oOldParent = oParent; 
 ...
 * @param {Element|String} [oBaseElement] 탐색 대상이 되는 DOM 요소. 지정한 요소의 하위 노드에서만 객체를 탐색한다. 생략될 경우 문서를 대상으로 찾는다.  id을 넣을 수 있다.
 ...
 * @throws {jindo.$Except.EITHER_ARRAY_OR_$A_OR_NULL_OR_UNDEFINED} 파라메터가 array, $A, null, undefined가 아닌 경우. 
 * @param {Void|Null|Undefined|Array|$A|NodeList|Arguments} [vArray] 없거나 null, undefined인 경우는 빈배열이 할당됨.  
 * @example
 ...
 * @description length() 메서드는 내부 배열 크기를 반환한다.
 * @memberOf $A#
 * @name length
 ...
 * @return {$A}	$A() 객체.
 * @throws {jindo.$Except.ONLY_FUNCTION_TYPE} 파라메터가 함수가 아닌 경우 예외상황이 발생된다.
...
jindo.$A.prototype.every = function(fCallback, oThis) {
	//-@@$A.every-@@//
	jindo.$Jindo.checkVarType(arguments, jindo.$A.checkVarTypeObj,"$A#every");
...
jindo.$A.prototype.some = function(fCallback, oThis) {
	//-@@$A.some-@@//
	jindo.$Jindo.checkVarType(arguments, jindo.$A.checkVarTypeObj,"$A#some");
...
테스트를 위해 우선 적용가능한 설정 객체가 존재하면 적용
	...
	switch (_opt.type) {
		...
		case "jsonp":
			//-@@$Ajax.jsonp-@@//
			if(!___ajax.JSONPRequest) throw new jindo.$Error(jindo.$Except.REQUIRE_AJAX+'jindo.$Ajax.JSONPRequest', "$Ajax");
...
jindo.$Ajax._validationOption = function(oOption,sMethod){
	...
	if(sType === "jsonp"){
		if(oOption["method"] !== "get") throw new __error(___except.CANNOT_USE_OPTION,sMethod);
	}else if(sType === "flash"){
		if(!(oOption["method"] === "get" || oOption["method"] === "post")) throw new __error(___except.CANNOT_USE_OPTION,sMethod,sMethod);
	}
	
	if(oOption["postBody"]){
		if(!(sType === "xhr" && (oOption["method"]!=="get"))){
			throw new __error(___except.CANNOT_USE_OPTION,sMethod);
		}
	}
	...
	for(var i = 0 ,l = aName.length; i < l ; i++){
		if(sProperty.indexOf(aName[i]) == -1) throw new __error(___except.CANNOT_USE_OPTION,sMethod);
	}
};
/**
 * @ignore
 ...
 * @param {Void|String|Object} oData 서버로 전송할 데이터.(string인 경우는 postbody가 true, type이 xhr, method가 get이 아는 경우만 사용가능)
 * @return {$Ajax} $Ajax() 객체.
 * @see $Ajax#option
...
jindo.$Ajax.prototype.request = function(oData) {
	...
	if (opt.postBody && sUpType == "XHR" && sUpMethod != "GET") {
		if(oArgs+"" == "4str"){
			...
			data = jindo.$Json(oArgs.sData).toString();	
		}else{
			data = null;
		}
	}else{
 ...
 * @param {Void|String} url
 * @return {Variant} URL의 값을 가져올 때는 문자열(String)을, 값을 설정할 때는 $Ajax() 객체를 반환한다.
 * @since 2.0.0 부터 지원
 ...
 * @return {$Ajax.Queue} 입력한 옵션을 설정한 $Ajax.Queue 객체를 반환한다.
 * @see $Ajax.Queue
 * @example
var oAjaxQueue = new $Ajax.Queue({
	useResultAsParam : true
 ...
 * @description option() 메서드는 $Ajax.Queue 객체에 설정한 옵션을 객체로 설정할 수 있다.
 * @memberOf $Ajax.Queue#
 * @name option@@2
 * @param {Object} oOption 옵션의 이름(String) 또는 하나 이상의 옵션을 설정한 객체(Object).
 * @return {$Ajax.Queue} 입력한 옵션을 설정한 $Ajax.Queue 객체를 반환한다.
 * @see $Ajax.Queue
 * @example
 ...
 * @description option() 메서드는 $Ajax.Queue 객체에 설정한 옵션 값을 확인한다.
 * @memberOf $Ajax.Queue#
 * @name option
 ...
 * @throws {jindo.$Except.EITHER_OBJECT_OR_$H_OR_NULL_OR_UNDEFINED} 파라메터가 object, $H, null, undefined가 아닌 경우. 
 * @example
 ...
 * object는 해시 그 자체를 가리킨다.
 * @param {Object} [oThis] 콜백 함수가 객체의 메서드일 때 콜백 함수 내부에서 this 키워드의 실행 문맥(Execution Context) 사용할 객체.
 * @return {$H} 콜백 함수의 수행 결과를 반영한 새로운 $H() 객체.
 * @throws {jindo.$Except.ONLY_FUNCTION_TYPE} 파라메터가 함수가 아닌 경우 예외상황이 발생된다.
 * @see $H#forEach
 * @see $H#filter
 * @see $H#map
 ...
 * @throws {jindo.$Except.ONLY_STRING_TYPE} 파라메터가 문자가 아닌 경우 예외상황이 발생된다.
 * @example
 ...
 * @param {Boolean} [bVisible] 해당 요소의 표시 여부.<br>입력한 파라미터가 true인 경우 display 속성을 설정하고 false인 경우에는 display 속성을 none으로 변경한다. boolean이 아닌 값이 들어온 경우는 ToBoolean한 결과를 가지고 비교한다.
 ...
 * @param {String} [sDisplay] display 속성에 지정할 값.<br>파라미터를 생략하면 태그별로 미리 지정된 기본 값이 속성 값으로 설정된다. 미리 지정된 기본 값이 없으면 "inline"으로 설정된다. 에러가 발생한 경우는 block으로 설정된다.
 ...
 * @return {Variant} opacity값을 확인할 때는 숫자(Number)을 반환하고, opacity값을 설정할 때는 값을 반영한 $Element() 객체를 반환한다.
 * @throws {jindo.$Except.ONLY_NUMBER_TYPE} 파라메터의 parseFloat한 결과가 number가 아닌 경우.
 * @example
 ...
 * @throws {jindo.$Except.EITHER_STRING_OR_OBJECT_OR_HASH} sName이 문자, 오브젝트 나 $Hash여야 한다.
 * @see $Element#attr
 * @example
<style type="text/css">
	#btn {
 ...
 * @throws {jindo.$Except.EITHER_STRING_OR_OBJECT_OR_HASH} sName은 문자,오브젝트 나 $Hash여야 한다.
 * @see $Element#css
 * @example
 ...
 * @param {Variant} vName class명(String), 하나 이상의 속성명과 불린값을 가지는 객체(Object) 또는 해시 객체($H()객체).<br>
 <ul>
	<li>파라미터가 문자열이면 다음과 같이 동작한다.
		<ul>
			<li>두 번째 파라미터인 bClassType를 생략하면 vName으로 지정한 클래스 속성이 있는지 확인한다.</li>
			<li>두 번째 파라미터인 bClassType를 불린 값으로 입력하면 true인경우는 vName의 클래스를 추가하고 false인 경우는 클래스를 삭제한다.</li>
		</ul>
	</li>
	<li>Object 혹은 $H 객체를 사용하면 두 개 이상의 class를 한번에 추가 및 삭제할 수 있다.</li>

</ul>
 * @param {Boolean} bClassType true인 경우는 클래스를 추가하고 false인 경우는 클래스를 삭제한다.
 * @return {Variant} 첫번째 인자만 있는 경우 해당 클래스가 있는지 여부의 불린 값을 반환하고, 아닌 경우는 $Element인스턴스를 반환한다.
 * @throws {jindo.$Except.EITHER_STRING_OR_OBJECT_OR_HASH} sName이 문자, 오브젝트 나 $Hash여야 한다.
 * @see $Element#addClass
 * @see $Element#removeClass
 * @example
 ...
 * @return {$Element} $Element() 객체.
 * @throws {jindo.$Except.ONLY_STRING_TYPE} 첫 번째 인자는 문자여야 한다.
 * @throws {jindo.$Except.EITHER_STRING_OR_FUNCTION} 두 번째 인자는 문자이거나 함수여야 한다.
 * @throws {jindo.$Except.ONLY_FUNCTION_TYPE} 세 번째 인자는 함수여야 한다.
 * @since 1.4.6부터 사용 가능.
 ...
 * @param {Function} fpCallback 이벤트 콜백함수
		...
		revisionCallback : function(sType, sEvent, fpCallback){
			if(sEvent=="mouseenter"||sEvent=="mouseleave"){
 ...
 * @ignore
		 ...
		 * @ignore
		 * @param {Object} oEventInfo 이벤트 정보객체
		 * @param {Function} vFilter 필터링하는 css선택자 혹은 필터함수
 ...
 * @throws {jindo.$Except.ONLY_NUMBER_TYPE} 파라메터는 숫자여야 한다.
 * @example
<style type="text/css">
 ...
 * @return {$Element} $Element() 객체.
 * @throws {jindo.$Except.EITHER_STRING_OR_ELEMENT_OR_$ELEMENT} 문자, 엘리먼트, $Element 중 하나여야 한다.
 * @see $Element#prepend
 * @see $Element#before
 * @see $Element#after
 ...
 * @throws {jindo.$Except.EITHER_NULL_OR_FUNCTION} 첫 번째 인자는 null, 함수 중 하나여야 한다.
 * @throws {jindo.$Except.ONLY_NUMBER_TYPE} 두 번째 인자는 숫자여야 한다.
 * @see $Element#child
 * @see $Element#prev
 * @see $Element#next
 * @see $Element#first
 ...
 * @param {Function|Null} [fCallback] 이전 형제 요소의 검색 조건을 지정한 콜백 함수.<br>파라미터를 생략하면 바로 이전 형제 요소를 반환하고, 파라미터로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true를 반환하는 이전 형제 요소를 반환한다. 이때 콜백 함수는 결과를 배열로 반환한다. 콜백 함수의 파라미터로 탐색 중인 이전 형제 요소의 $Element() 객체가 입력된다.null인 경우 모든 형제노드를 반환한다.
 * @return {Variant} 바로 이전 형제 요소가 담긴 $Element() 객체 혹은 조건을 만족하는 이전 형제 요소의 배열(Array).<br>바로 이전 형제 요소를 반환할 때는 $Element() 객체를 반환하고 그 이외에는 $Element() 객체를 원소로 갖는 배열로 반환한다. 만약 엘리먼트가 없으면 null을 반환.
 * @throws {jindo.$Except.EITHER_NULL_OR_FUNCTION}  인자는 null, 함수 중 하나여야 한다.
 * @see $Element#parent
 * @see $Element#child
 * @see $Element#next
 * @see $Element#first
 ...
 * @return {Variant} 바로 다음 형제 요소가 담긴 $Element() 객체 혹은 조건을 만족하는 다음 형제 요소의 배열(Array).<br>바로 다음 형제 요소를 반환할 때는 $Element() 객체를 반환하고 그 이외에는 $Element() 객체를 원소로 갖는 배열로 반환한다.만약 엘리먼트가 없으면 null을 반환.
 * @throws {jindo.$Except.EITHER_NULL_OR_FUNCTION}  인자는 null, 함수 중 하나여야 한다.
 * @see $Element#parent
 * @see $Element#child
 * @see $Element#prev
 ...
 * @throws {jindo.$Except.ONLY_STRING_TYPE} 첫 번째 인자는 문자여야 한다.
 * @throws {jindo.$Except.EITHER_OBJECT_OR_UNDEFINED} 두 번째 인자는 오브젝트이거나 undefined여야 한다.
...
jindo.$Element.prototype.prependHTML = function(sHTML) {
	...
	return ___element.insertAdjacentHTML(this,sHTML,"afterBegin","firstChild",jindo.$Fn(function(oEle){
		___element._prepend(this._element,oEle);
...
jindo.$Element.prototype.hasEventListener = function(sEvent){
	if(this._key){
		return !!jindo.$Element.eventManager.hasEvent(this._key, sEvent);
	}
	return false;
};
 ...
 * @return {$Fn} $Fn() 객체
 * @throws {jindo.$Except.EITHER_STRING_OR_FUNCTION} 파라메터가 문자이거나 함수여야 한다.
 * @see $Fn#toFunction
 * @example
func : function() {
       // code here
...
jindo.$Fn = function(func, thisObject) {
	...
	var oArgs = jindo.$Jindo.checkVarType(arguments, {
		'4str' : ['func:String+'],
		'4str2' : ['func:String+', "thisObject:Variant"],
		'4fun' : ['func:Function+'],
 ...
 * @param {Variant} vElement 이벤트 핸들러를 할당할 요소(Element,$Element) 또는 요소로 이루어진 배열(Array,$A). id,tag을 넣을 수 있다(String).
 * @param {String} sEvent 이벤트 종류
 * @param {Boolean} [bUseCapture] 캡쳐링(capturing) 사용 여부(1.4.2 버전부터 지원). 사용하면 true 사용하지 않으면 false를 입력한다.
 * @see $Fn#detach
 * @return {$Fn} 생성된 $Fn() 객체.
 * @throws {jindo.$Except.EITHER_STRING_OR_DOM_OR_$ELEMENT_OR_ARRAY_OR_$A} 첫 번째 인자가 id, tag, element, $Element, Array, $A이 아닌 경우.
 * @throws {jindo.$Except.ONLY_STRING_TYPE} 두 번째 인자가 string이 아닌 경우.
 ...
 * @param {Boolean} bIsScrollbar true이면 scroll속성을 알 수 있다.(기본 값은 false)
 <table>
 ...
 * @param {Variant} vElements 문서에서 DOM 요소를 찾기 위한 CSS 선택자(CSS Selector) 혹은 ID(String), $Element() 객체를 원소로 같는 배열(Array,$A)를 입력한다.인자가 없거나 null인 경우는 빈배열이 할당된다.
 * @throws {jindo.$Except.$ELEMENTLIST_OPTION} 인자가 배열, 문자, $A, null, undefined가 아닌 경우에 발생한다.
 ...
 * @param {Number} nTimes 반복할 횟수.(기본값은 1)
 ...
 * @param {String} sName 입력 요소의 이름. 이 파라미터로 여러 입력 요소의 활성화 여부를 지정한 객체및 $H를 입력할 수 있다. 이와 관련한 내용은 예제를 참고한다.
 * @param {Boolean} [bEnable] 활성화 여부.
 * @throws {jindo.$Except.EITHER_STRING_OR_OBJECT_OR_HASH} 파라메터가 문자, 오브젝트, $H가 아닌 경우 예외상황이 발생한다.
 ...
 * @param {String|Function} [sTargetName] 전송 대상. 이 파라미터를 생략하면 &lt;form&gt; 요소의 target 속성에 지정된 대상으로 정보를 전송한다.
 * @param {Function} [fValidation] &lt;form&gt; 요소의 데이터를 검증할 함수. 이 함수의 파라미터로 $Form() 객체가 감싸고 있는 원본 &lt;form&gt; 요소가 입력된다.
 * @return {$Form} 데이터를 제출한 $Form() 객체.
 * @throws {jindo.$Except.EITHER_STRING_OR_FUNCTION} 첫 번째 인자가 문자나 함수가 아닌경우 예외상황 발생.
 * @throws {jindo.$Except.EITHER_STRING_OR_FUNCTION} 두 번째 인자가 함수가 아닌 경우 예외상황 발생.
 * @example
var form = $Form(el);
form.submit();
form.submit('foo');
 ...
 * @throws {jindo.$Except.ONLY_FUNCTION_TYPE} 첫 번째 인자가 함수가 아닌 경우 예외상황.
 * @example
var form = $Form(el);
form.reset(); 
 ...
 * @param {Object} oDdata 템플릿에 적용할 데이터를 가진 객체. 템플릿에 데이터를 적용할 부분은 객체의 속성(property) 이름으로 찾는다.
 ...
 * @param {Variant} vName names속성 이름(String), 하나 이상의 names속성과 값을 가지는 객체(Object) 또는 해시 객체($H()객체).<br>
 <ul>
	<li>파라미터가 문자열이면 다음과 같이 동작한다.
		<ul>
			<li>두 번째 파라미터인 vValue를 생략하면 vName으로 지정한 names값을 가져온다.</li>
			<li>두 번째 파라미터인 vVlaue를 입력하면 vName으로 지정한 names값을 vValue로 설정한다.</li>
		</ul>
	</li>
	<li>Object 혹은 $H 객체를 사용하면 두 개 이상의 names속성을 한번에 설정할 수 있다.</li>

</ul>
 * @param {Array} aValue names속성에 설정할 값.
 * @return {Variant} names 속성 값을 확인할 때는 값을 반환하고, names속성 값을 설정할 때는 값을 반영한 인스턴스를 반환한다.
 * @throws {jindo.$Except.EITHER_STRING_OR_OBJECT_OR_HASH} sName이 문자, 오브젝트 나 $Hash여야 한다.
 * @see $Date.names
 ...
 * @param {Date|$Date} [oDate]  비교할 날짜.
 * @param {String} [sOption]  비교할 단위를 지정.(기본 값은 천 분의 1초까지 비교하며, y, m, d, h, i, s등을 넣을 수 있다. 각 옵션은 y는 년, m은 달, d는 날, h는 시, i는 분, s는 초로 한다)
 * @returns {Number} 비교 단위를 기준으로 차이를 숫자로 반환한다.
 * @example
