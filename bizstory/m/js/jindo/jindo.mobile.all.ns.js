...
 * NAVER_Library:Jindo-2.8.0;JavaScript Framework;
...
jindo.$Jindo.checkVarType._throwException = function(aArgs, oRules, sFuncName) {
	...
	if (/(\$\w+)#(\w+)?/.test(sFuncName)) {
        sURL = 'http://jindo.dev.naver.com/docs/jindo/archive/Jindo2-2.8.0/mobile/ko/classes/' + encodeURIComponent(RegExp.$1) + '.html' + "#method_"+RegExp.$2;
    }
    
//     
	// if (/(\$\w+)(#\w+)?/.test(sFuncName)) {
		// sURL = 'http://jindo.dev.naver.com/docs/jindo/archive/Jindo2-2.8.0/mobile/ko/classes/' + encodeURIComponent(RegExp.$1) + '.html' + RegExp.$2;
	// }
...
테스트를 위해 우선 적용가능한 설정 객체가 존재하면 적용
	...
	if (window["__"+jindoName+"_callback"] === undefined) {
        ...
        window["__"+jindoName+"2_callback"] = [];
	}
	...
	send  : function(data) {
		...
		window["__"+jindoName+"_callback"][info.id] = function(data){
			try {
				...
				delete window["__"+jindoName+"2_callback"][info.id];
			}
		};
		window["__"+jindoName+"2_callback"][info.id] = function(data){
            window["__"+jindoName+"_callback"][info.id](data);
        };
...
function defaultSort(oArgs,that,type){
    var aSorted = [];
...
jindo.$H.prototype.sort = function(fpSort) {
    ...
    this["__jindo_sorted_index"] = defaultSort(oArgs,this,"val"); 
    return this;
};
...
jindo.$H.prototype.ksort = function(fpSort) {
    ...
    this["__jindo_sorted_index"] = defaultSort(oArgs,this,"key");
    return this;
};
...
/**
	...
	@param {Function+} [fCallback] vFilter에 지정된 함수가 true를 반환하는 경우 실행할 콜백 함수. 콜백 함수의 첫 번째 파라미터는 HTML 요소 자신이고, 두 번째 파라미터는 이벤트가 발생한 HTML 요소가 입력된다.
