...
//------------------------------------ 팝업등록폼 열기
	...
	$(document).on('click', "[data-bs-target='#kt_modal_position']", function(e) {
		
		$('#list_idx').val($(this).attr('data-bs-idx'));
	...
	$(document).on('click', ".kt_user_button", function(e) {
		const link_form = local_dir + '/bizstory/msg/popup_msg_form.php';
		const target_id = $(this).attr('data-bs-target');
		let idx = $(this).attr('data-bs-idx');
...
//------------------------------------ 등록, 수정
	...
	function data_view_open(idx)
	{
		if(!idx) return false;

		$("#popup_notice_view").hide();
		$('#list_idx').val(idx);
		$('#list_sub_type').val('viewform');
		location.href = '?' + $('#listform').serialize();
	}
