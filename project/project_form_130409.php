...
<div class="ajax_write">
	<div class="ajax_frame">
					...
					?>
										<li class="mem_name">
											<label for="<?=$checkbox_str;?>">
												<input type="checkbox" name="project_member_idx[]" id="<?=$checkbox_str;?>" value="<?=$mem_data['mem_idx'];?>" class="type_checkbox"<?=$checked;?><?=$disabled;?> title="<?=$mem_data['mem_name'];?>" onclick="select_member()" />
												<?=$mem_data['mem_name'];?>
											</label>
										</li>
					<?
		...
		?>
				<tr>
					<th><label for="file_fname">파일-작업중(사용안됨)</label></th>
					<td colspan="3">
						<div style="border:1px #ccc solid;width:100%;">
							<object id="XFile" width="100%" height="100" classid="clsid:16ADD49A-D148-401E-877C-1B2D1DDEBC7E"
									codebase="<?=$local_dir;?>/bizstory/filecenter/xupload/xfile.ocx">
								<param name="ServerURL"    value="http://<?=$filecneter_url;?>/filemanage/xupload/upload_activex_ok.php" />
								<param name="Filter"       value="모든 파일(*.*)|*.*|" />
								<param name="MaxFileCount" value="<?=$file_max_cnt;?>" />
								<param name="MaxFileSize"  value="<?=$file_max_size;?>" />
						...
						<div style="text-align:left;padding-top:5px;">
							...
							<span class="btn_big"><input type="button" value="파일 삭제" onclick="XFile.Remove()" /></span>
							<span class="btn_big"><input type="button" value="전체 삭제" onclick="XFile.RemoveAll()" /></span>
							<span class="btn_big"><input type="button" value="파일 전송" onclick="XFile.Upload()" /></span>
						</div>
					</td>
				</tr>
		<?
		...
		?>
			...
			?>
					...
					<span class="btn_big fl"><input type="button" value="등록취소" onclick="close_data_form()" /></span>
			...
			?>
					...
					<span class="btn_big fl"><input type="button" value="수정취소" onclick="close_data_form()" /></span>
...
<form name="isForm" method="post">
	<input type="hidden" name="comp_idx" id="xfile_comp_idx" value="<?=$code_comp;?>" />
	<input type="hidden" name="part_idx" id="xfile_part_idx" value="<?=$code_part;?>" />
	<input type="hidden" name="mem_idx"  id="xfile_mem_idx"  value="<?=$code_mem;?>" />
	<input type="hidden" name="add_name" id="xfile_add_name" value="<?=time();?>" />
	<input type="hidden" name="pro_idx"  id="xfile_pro_idx"  value="<?=$pro_idx;?>" />
	<input type="hidden" name="pro_code" id="xfile_pro_code" value="<?=$data['pro_code'];?>" />
</form>
		...
		if (action_num == 0)
		{
			...
			$.ajax({
				...
				<?
					...
					{
				?>
						//var add_name = $('#xfile_add_name').val() + '_' + $('#xfile_comp_idx').val() + '_' + $('#xfile_part_idx').val() + '_' + $('#xfile_mem_idx').val();
						var add_name = '';
						filecenter_project_folder(msg.f_idx, add_name);
						close_data_form();
				<?
					}
					else
