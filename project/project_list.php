...
// 검색
    ...
    $file_where = str_replace("pro.", "", $where);
    
	if ($stext != '' && $swhere != '') $where .= " and " . $swhere . " like '%" . $stext . "%'";
    
    $search_scharge = $scharge;
    $search_sapply = $sapply;
    $search_sreg = $sreg;
    if ($search_scharge != '') {
        $where .= " and concat(',', pro.charge_idx, ',') like '%," . $search_scharge . ",%' ";
    }
    if ($search_sapply != '') {
        $where .= " and pro.apply_idx in (" . $search_sapply . ")";
    }
    if ($search_sreg != '') {
        $where .= " and pro.reg_id in (" . $search_sreg . ")";
    }
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 정렬
	if ($sorder1 == '' || $sorder2 == '')
...
// 목록, 페이지관련
	...
	$list = project_list_info('list', $where, $file_where, $orderby, $page_num, $page_size);
...
<?
	...
	else
	{
		...
		foreach($list as $k => $data)
		{
			...
			{
				if ($auth_menu['view'] == "Y") {
				    $btn_view = "view_open('" . $data['pro_idx'] . "')";
				    $link_hash = "#v" . time() . $data['pro_idx'];
				} else {
				    $btn_view = "check_auth_popup('view')";
                    $link_hash = "javascript:void(0);";
                }
