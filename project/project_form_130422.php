	...
	if ($part_list['total_num'] > 0)
	{
		...
		foreach ($part_list as $part_k => $part_data)
		{
			if (is_array($part_data))
			{
				...
				$change_str .= '
				<li class="first" id="chargelist_' . $part_idx . '">
					<label for="' . $chk_str . '">
				...
				if ($group_list['total_num'] > 0)
				{
					...
					foreach ($group_list as $group_k => $group_data)
					{
						...
						// 그룹별 직원
							...
							{
								$change_str .= '
						<li class="second" id="chargelist_' . $part_idx . '_' . $csg_idx . '">
							<label for="' . $chk_strg . '">
							...
							<ul id="chargeul_' . $part_idx . '_' . $csg_idx . '">';
								foreach ($mem_list as $mem_k => $mem_data)
								{
									if (is_array($mem_data))
									{
										...
										$change_str .= '
								<li class="mem_name" id="chargelist_' . $part_idx . '_' . $csg_idx . '_' . $mem_idx . '">
...
// 담당자 펼침
	var now_folder_id = 'chargelist_<?=$chk_up;?>';

	var fsidebar = document.getElementById("charge_navi");
	if (fsidebar)
	{
