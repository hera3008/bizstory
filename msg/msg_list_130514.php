...
<?
	...
	else
	{
		...
		foreach ($msg_reg as $k => $data)
		{
			...
			$del_yn   = $msg_data[$k]['del_yn'];

			if ($del_yn == 'Y') $del_str = "-퇴사직원";
			else $del_str = "";
?>
				<tr>
					<td><span class="num"><?=$i;?></span></td>
					<td><div class="left"><img class="photo" src="/bizstory/images/tfuse-top-panel/no_member.jpg" alt="" height="22px" width="22px"><a href="javascript:void(0);" onclick="msg_list_data('<?=$mem_idx;?>')"><?=$mem_name;?></a> (<?=number_format($msg_num);?>) <span class="num3"><em>15</em></span><?=$del_str;?></div></td>
				</tr>
<?
				$num--;
				$i++;
