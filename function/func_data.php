...
//-------------------------------------- 기관 직책
	function company_staff_duty_data($query_type, $where = '', $orderby = '', $page_num = '', $page_size = '', $del_type = 1)
	{
		if ($orderby == '') $orderby = "csd.sort asc";
		if ($del_type == 1) $where = "csd.del_yn = 'N'" . $where;
		else $where = "1" . $where;
	...
	function company_info_data($query_type, $where = '', $orderby = '', $page_num = '', $page_size = '', $del_type = 1)
	{
		...
		$query_page = "
			...
			from
				...
				left join company_class code on code.del_yn = 'N' and code.code_idx = comp.comp_class_sub
...
//-------------------------------------- 해당기본값 셋팅
	function default_code_setting($comp_idx, $part_idx)
	{
		...
		$company_staff_group_chk_data = query_view("select count(csg_idx) as cnt from company_staff_group where comp_idx = '" . $comp_idx . "'");
		if ($company_staff_group_chk_data['cnt'] == 0){ // 값이 없을 경우 등록
	// 직원그룹
		...
		$code_receipt_status_chk_data = query_view("select count(code_idx) as cnt from code_receipt_status where comp_idx = '" . $comp_idx . "'");
		if ($code_receipt_status_chk_data['cnt'] == 0){ // 값이 없을 경우 등록
	// 접수상태 - 접수, 접수승인, 작업중, 완료, 보류, 취소
		...
		$code_project_status_chk_data = query_view("select count(code_idx) as cnt from code_project_status where comp_idx = '" . $comp_idx . "'");
		if ($code_project_status_chk_data['cnt'] == 0){ // 값이 없을 경우 등록
	// 프로젝트상태 - 대기, 진행, 취소, 보류, 완료
		...
		$code_sche_class_chk_data = query_view("select count(code_idx) as cnt from code_sche_class where comp_idx = '" . $comp_idx . "'");
		if ($code_sche_class_chk_data['cnt'] == 0){ // 값이 없을 경우 등록
	// 일정종류 - 개인, 업무, 미팅
...
// 회원가입
	function regist_member($mem_id, $param)
	{
		global $_SESSION, $sess_str;

		if($_SESSION[$sess_str . '_mem_idx']) $reg_id = $_SESSION[$sess_str . '_mem_idx'];
		else $reg_id = 1;
